...
\begin{titlepage}	\vfill	
	\begin{center}		{\instituicaoformat Instituto de Ciências Matemáticas e de Computação \\ Universidade de São Paulo \\ ICMC - USP} \\[1cm] 
	
		\begin{figure} [ht]
		\centering
		\includegraphics[width=0.3\textwidth]{images/icmc.png}
		\end{figure}
		
		\vspace{2cm}
		
		{\tituloformat Avaliação de Sistemas de Arquivos \\  \normalsize \textcolor{light-gray}{(Ext4, ReiserFS e XFS)}}\\
		
		\vfill
		
		{Trabalho da disciplina de Sistemas Operacionais \\ Pós-graduação 2011}\\[2cm]
		
		\vfill
		
		{\autorformat Fausto Guzzo da Costa \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ nUsp: 5230736\\ Filipe Del Nero Grillo \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ \ nUsp: 5378140 \\ Vinicíus Augusto Tagliatti Zani \ \ \ \ \ \ \ nUsp: 5118935}
...
Diversas propostas têm sido feitas para gestão interna de arquivos
de sistemas operacionais. No âmbito de sistemas operacionais de código aberto,
Ext4, XFS e ReiserFS foram lançados ao público como sistemas de arquivos
eficientes para o sistema operacional Linux, e têm tido adoção crescente como
consequência das funcionalidades que oferecem para realização de gestão de
arquivos.
Neste trabalho, foi realizado uma avaliação de desempenho entre os sistemas de
arquivos Ext4, XFS e ReiserFS medindo-se o \emph{throughput} das operações de
escrita, leitura, leitura aleatória e escrita aleatória. Os experimentos
contavam com a variação de tamanho de bloco da partição, quantidade de memória
RAM do sistema e o tamanho dos arquivos que foram utilizados nas operações
descritas anteriormente. Ao fim da realização dos experimentos foi possível
concluir que cada uma das abordagens tem pontos fortes e fracos, que devem ser
levados em consideração de acordo com o tipo de uso que se fará do sistema de
arquivos.
\end{resumo}
...
tornem obsoletas. Tais mudanças podem ser observadas no âmbito de \emph{hardware} e
\emph{software}. Para os softwares, mais especificamente dentro do domínio de sistemas
...
arquivos físicos --- mapeados em \emph{hardware}.
...
sistema operacional Linux, e têm tido adoção crescente como
...
Este trabalho tem como objetivo avaliar o desempenho dos sistemas de arquivos
Ext4, XFS e ReiserFS em comparação uns aos outros. Esta avaliação pretende
explicitar vantagens e desvantagens destes sistemas de arquivos e esclarecer em
que situações cada um possui melhor desempenho.
...
de blocos de 4 KB (experimentos 9 à 12). Entretanto, quando o bloco passa a ser de 1 KB (experimentos 13 à 16), nada pode se
...
desempenho de escrita (MB/s) foi obtido utilizando XFS com blocos de 4 KB e
arquivos de tamanho de 64 MB, onde nada pode se afirmar sobre a memória, pois
para ambos os valores de 1 GB e 2 GB (experimentos 10 e 12, respectivamente) as taxas de escrita com seus respectivos
...
arquivos são de 64 MB (experimentos 7 e 8, respectivamente). Apesar da diferença ser significativa para o primeiro
...
KB foram utilizados com 1 GB de memória com arquivos de tamanho de 64 KB (experimento 9). Nesse
experimento, o Ext4 teve um desempenho \emph{médio} superior ao XFS.
...
Figura \ref{fig:barra1_randwrite}) o Ext4 teve um desempenho visilmente superior ao XFS quando os blocos eram de 4 KB, a memória de 1 GB e arquivos de 64 KB (experimento 1). Uma diferença um pouco menor, com \emph{throughput} similar, pode ser
notada com os mesmos fatores acima, porém tamanho de arquivo de 64 MB (experimento 2).
...
de ambos os sistemas de arquivos.
...
a operação de leitura --- houve resultados muito parecidos em quase todos os experimentos, com exceção do experimento 7, onde foi testado com 2 GB de memória e arquivos de 64 KB, e houve um \emph{throughput} bem maior do ReiserFS.
...
de 2 GB. Quando há mais memória disponível --- fator importante para
...
\ref{fig:barra2_randwrite}), nada se pode afirmar
...
arquivos grandes são utilizados.
...
A partir da Figura \ref{fig:barra3_write}, podemos concluir que o XFS desempenha
...
Na operação de leitura, mostrada na Figura \ref{fig:barra3_read}, há um ganho em
todas as comparações do ReiserFS. Nota-se, principalmente, um ganho nos
experimentos 5 e 7, onde foi considerado o teste com tamanho de arquivos
pequenos, de 64 KB; e houve aumento do desempenho com o aumento da memória.
...
As operações de escrita randômica e leitura randômica geraram resultados parecidos, como é visto nas Figuras
...
experimentos 5 e 7 pelo ReiserFS em ambas operações. Estes experimentos utilizavam tamanhos de arquivos
pequenos, de 64 KB e a memória principal variável. Note que a memória influencia
muito nesses resultados, aumentando o desempenho.
...
Por fim, a quantidade de memória principal é também um fator de importância
para operação de leitura. Nesse caso, com o aumento da memória
disponível há um ganho no desempenho de ambos os sistemas de arquivos.
...
\begin{itemize}
  ...
  randômica - há uma queda no desempenho quando utilizado arquivos grandes.
\end{itemize}
...
\begin{itemize}
  ...
  \item Para as operações de leitura randômica e escrita randômica, o Ext4 e o ReiserFS obtiveram desempenhos similares,
...
\begin{itemize}
  \item O sistema de arquivos XFS obteve melhor resultado na operação de escrita
  em todos os casos;
  ...
  \item As operações de escrita randômica e leitura randômica tiveram \emph{throughput} parecidos em todos os casos, onde o ReiserFS tendo vantagem principalmente com arquivos de 64 KB;
...
O objetivo dessa avaliação de desempenho foi de comparar os sistemas de arquivos
Ext4, XFS e ReiserFS para operações de leitura, escrita, leitura aleatória e
escrita aleatória, medindo-se o \emph{throughput} das operações. Foram adotados
como fatores tamanho de bloco da partição, quantidade de memória RAM disponível
e tamanho dos arquivos utilizados nas operações.
Ao término dos experimentos, foi possível concluir que blocos maiores nas
partições oferecem maior desempenho para as operações avaliadas. Em relação a
sistemas de arquivos, o XFS se mostrou mais eficiente quando há mais operações
de escrita, principalmente com arquivos maiores. Por outro lado, o sistema de
arquivos ReiserFS se mostrou mais eficiente em operações de leitura,
principalmente com arquivos menores. Por fim, o sistema de arquivos Ext4
realizou todas as operações em tempos médios, não apresentando quedas
significativas de desempenho.

Assim, podemos concluir que há casos onde cada sistema de arquivo apresenta um
melhor desempenho, e como consequência sua adoção é mais interessante. O XFS,
por possuir bom desempenho em escrita, pode ser utilizado como exemplo em
sistemas que realizam \emph{backup} de dados, ou grandes bases de dados que
sofrem poucas consultas. Já o ReiserFS, por possuir bom desempenho de leitura,
pode ser utilizado em sistemas que servem conteúdo, como servidores \emph{web}.
Entretanto, vale ressaltar que o suporte a tal sistema de arquivos foi
descontinuado, e a adoção passa a estar sujeita aos riscos inerentes a este
fato.
Por fim, o Ext4 passa então a ser aconselhável em casos onde há um equilíbrio
entre operações de leitura e escrita, pois seu desempenho não é demasiadamente
afetado por operações específicas. Assim sendo, um usuário comum poderia
adotá-lo como forma mais segura de garantir um desempenho aceitável.

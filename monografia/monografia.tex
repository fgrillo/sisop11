...
arquivos.
Neste trabalho, foi realizado uma avaliação de desempenho entre os sistemas de arquivos Ext4, XFS e ReiserFS medindo-se o \emph{throughput} das operações de escrita, leitura, leitura aleatória e escrita aleatória. Os experimentos contavam com a variação de tamanho de bloco da partição, quantidade de memória RAM do sistema e o tamanho dos arquivos que foram utilizados nas operações descritas anteriormente. Ao fim da realização dos experimentos
foi possível concluir que cada uma das abordagens tem pontos fortes e fracos,
que devem ser levados em consideração de acordo com o tipo de uso que se fará do
sistema de arquivos.
\end{resumo}
...
\begin{itemize}
  \item Os sistemas de arquivos tem um desempenho melhor utilizando blocos de 4 KB do que 1 KB;

  \item O XFS mostrou ser o melhor sistema de arquivos em escrita, principalmente com 2 GB de memória e arquivos de 64 MB;

  \item O ReiserFS realizou os maiores valores de leitura, principalmente de arquivos pequenos;

  \item O Ext4 fica em segundo em todas as operações, sendo um sistema de arquivos que realiza todos os tipos de operação com bom \emph{throughput}.
\end{itemize}
